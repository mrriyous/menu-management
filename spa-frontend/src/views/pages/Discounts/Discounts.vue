...
<template>
    <v-row>
        <v-col cols="12" md="12">
            <UiParentCard title="Discounts"> 
                <template v-slot:action>
                    <v-btn @click="addItem()" color="primary">
                        <PencilPlusIcon stroke-width="1"></PencilPlusIcon> Add Discount
                    </v-btn>
                </template>
                ...
                <table class="styled-table mb-5">
                    ...
                    <tbody>
                        ...
                        <tr v-for="(discount, index) in discounts" :key="index" :class="{ 'loading-row': loading }">
                            <td>{{ discount.code }}</td>
                            <td>{{ discount.name }}</td>
                            <td>{{ discount.discount_type }}</td>
                            <td>{{ discount.discount_percentage }}</td>
                            <td>
                                <span v-if="discount.discountable != undefined">
                                    {{ discount.discountable.code }} - {{ discount.discountable.name }}
                                </span>
                            </td>
                            <td>
                                <div class="d-flex gap-3">
                                    <v-btn @click="editItem(discount)" color="primary" icon size="x-small" flat>
                                        <EditIcon stroke-width="1"  />
                                    </v-btn>
                                    <v-btn @click="deleteItem(discount)" color="error" icon size="x-small" flat>
                ...
                <v-pagination v-model="currentPage" :length="totalPages" v-if="discounts.length > 0"></v-pagination>
            </UiParentCard>
        </v-col>
    </v-row>
</template>
    ...
    methods: {
        ...
        async getData() {
            try {
                ...
                const response = await axios.get(`${apiUrl}/discounts?page=${this.currentPage}&search=${this.searchQuery}`, {
                    headers: {
                ...
                this.totalPages = response.data.discounts.last_page;
                this.loading = false;
            } catch (error) {
                this.loading = false;
                console.log(error);
        ...
        async deleteItem(discount) {
            const confirm = window.confirm("Are you sure to delete this discount?");
            if (!confirm) {
                return;
            }
            ...
            try {
                ...
                const response = await axios.post(`${apiUrl}/discounts/${discount.id}`, {
                    _method: "DELETE"
                }, {
                    headers: {
                        Authorization: `Bearer ${bearerToken}`
